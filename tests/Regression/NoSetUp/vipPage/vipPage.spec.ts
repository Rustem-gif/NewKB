...
for (const [status, creds] of Object.entries(VIP_USERS)) {
  test.describe.skip(`Visual tests for ${status}`, () => {
    ...
    test(`Current Status Image - ${status}`, async ({ baseURL }) => {
      await expect(vipPage.getCurrentStatusImage).toHaveScreenshot(
        `current-status-${status}${baseURL}.png`,
        { maxDiffPixels: 1000, maxDiffPixelRatio: 1 }
      );
    });

    test(`VIP Page Logo - ${status}`, async ({ baseURL }) => {
      await expect(vipPage.getVipPageLogo).toHaveScreenshot(`vip-logo-${status}${baseURL}.png`, {
        maxDiffPixels: 1000,
        maxDiffPixelRatio: 1,
      });
    });
    ...
    test(`Card List - ${status}`, async ({ baseURL }) => {
      await expect(vipPage.getCardList).toHaveScreenshot(`card-list-${status}${baseURL}.png`, {
        maxDiffPixels: 1000,
        maxDiffPixelRatio: 1,
      });
    });
    ...
    if (status === 'guest') {
      ...
      test(`Page Level Logo - ${status}`, async ({ baseURL }) => {
        await expect(vipPage.getPageLevelLogo).toHaveScreenshot(
          `page-logo-${status}${baseURL}.png`,
          { maxDiffPixels: 1000, maxDiffPixelRatio: 1 }
        );
      });
    }
