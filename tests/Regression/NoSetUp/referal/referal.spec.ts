...
test.describe('Referral', () => {
  ...
  test('Check the /referral_program. Ref user', async () => {
    ...
    await test.step('Check link of the page', async () => {
      expect(await refPage.getPageUrl()).toEqual(
        `${test.info().project.use.baseURL}${LINKS.referalProgram}`
      );
    });
    ...
    await test.step('Check link of the page', async () => {
      ...
      expect(await refPage.getPageUrl()).toMatch(
        new RegExp(`${test.info().project.use.baseURL}${LINKS.refProgramProfile}`)
      );
    });
