...
import { getVpnController } from '../../helpers/vpnControllerFactory';
...
test.describe('Deposit Flow Test', () => {
  ...
  test('Verify deposit flow AU credit card', async ({ browser }) => {
    ...
    const context = await browser.newContext({
      userAgent:
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) kingBillyleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    });
    ...
    await expect(page).toHaveScreenshot('au_credit_card.png', {
      maxDiffPixels: 2000,
    ...
    await test
      .info()
      .attach('au_credit_card.png', { body: auCreditCardScreenshot, contentType: 'image/png' });
    await context.close();
  ...
  test('Verify deposit flow AU Neoserf', async ({ browser }) => {
    ...
    expect(url.includes('pay2.secure-neosurf.com') || url.includes('pay.neosurf.com')).toBe(true);
    expect(neoserfPage.getNeosurfPageLogo).toBeVisible();
    await expect(page).toHaveScreenshot('au_neoserf.png', {
      maxDiffPixels: 2000,
      maxDiffPixelRatio: 0.5,
    });
    ...
    await test
      .info()
      .attach('au_neoserf.png', { body: auNeoserfScreenshot, contentType: 'image/png' });
    await context.close();
  ...
  test('Verify deposit flow NZ credit card', async ({ browser }) => {
    ...
    await kingBilly.depModal.clickOnDepositButton();
    await expect(page).toHaveScreenshot('nz_credit_card.png', {
      maxDiffPixels: 2000,
      maxDiffPixelRatio: 0.5,
    });
    ...
    await test
      .info()
      .attach('nz_credit_card.png', { body: nzCreditCardScreenshot, contentType: 'image/png' });
    await kingBilly.depModal.page.waitForTimeout(60000);
    expect(kingBilly.depModal.getDepModalError).toBeVisible({ timeout: 5000 });
    await context.close();
  ...
  test('Verify deposit flow NZ paysafecard', async ({ browser }) => {
    ...
    await expect(page).toHaveScreenshot('nz_paysafecard.png', {
      maxDiffPixels: 2000,
      maxDiffPixelRatio: 0.5,
    });
    ...
    await test
      .info()
      .attach('nz_paysafecard.png', { body: nzPaysafecardScreenshot, contentType: 'image/png' });
    await context.close();
  ...
  test('Verify deposit flow CA interact', async ({ browser }) => {
    ...
    await expect(kingBilly.mainPage.page.locator('.otherPayments > p')).toContainText(
      'Select your bank'
    );
    await expect(page).toHaveScreenshot('ca_interac.png', {
      maxDiffPixels: 2000,
      maxDiffPixelRatio: 0.5,
    });
    ...
    await test
      .info()
      .attach('ca_interac.png', { body: caInteracScreenshot, contentType: 'image/png' });
    await context.close();
  ...
  test('Verify deposit flow CA credit card', async ({ browser }) => {
    ...
    await expect(page).toHaveScreenshot('ca_credit_card.png', {
      maxDiffPixels: 2000,
      maxDiffPixelRatio: 0.5,
    });
    await context.close();
