...
export default class Header extends BaseComponent {
  ...
  readonly filterProviderButton: Locator = this.page
    .locator('.games-search-filter-block__header')
    .filter({ hasText: /^Provider$/ });
  readonly filterCategoriesButton: Locator = this.page
    .locator('.games-search-filter-block__header')
    .filter({ hasText: /^Category$/ });

  readonly langDropdownItem = (language: string) =>
    this.page.locator('.select-language-icons-with-code__link').filter({ hasText: language });
  ...
  async getListOfFilterProviders(): Promise<Array<string>> {
    return await this.page.evaluate(() => {
      const filterProviders = document.querySelectorAll(
        '.games-search-filter-block__header--active + div.collapse.collapse--entered  .games-search-filter-item '
      );
      if (!filterProviders) {
  ...
  async getListOfFilterCategories(): Promise<Array<string>> {
    return await this.page.evaluate(() => {
      const filterCategories = document.querySelectorAll(
        '.games-search-filter-block__header--active + div.collapse.collapse--entered .games-search-filter-block__values .games-search-filter-item'
      );
      if (!filterCategories) {
        throw new Error('Providers not found in the filter, something went wrong, better debug');
      }
  ...
  async getCurrencies() {
    return await this.page.evaluate(() => {
      const currencyNodes = document.querySelectorAll(
        '.balance-select__dropdown > [role= "option"] > span'
      );
