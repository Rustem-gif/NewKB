    ...
    switch (bonusType) {
      ...
      case 'kingsCoins': {
        ...
        if (await this.fiatCard.isVisible()) {
          const fiatTitle = await this.fiatCard
            .locator('.Block--page-bonuses-store-card-money-fiat__title')
            .innerText();
          const fiatPrice = await this.fiatCard
            .locator('.Block--page-bonuses-store-card-money-fiat__description')
            .innerText();
