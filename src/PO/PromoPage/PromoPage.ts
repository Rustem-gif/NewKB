...
export default class PromoPage extends BasePage {
  ...
  private promoCardDepositButton: Locator = this.page.locator(
    '.promo-modal__button.deposit-button'
  );
  ...
  private tournamentShowMoreButton = (index: number) =>
    this.page.locator(`.a.tourn-item__button.link-btn:nth-of-type(${index})`);
  ...
  async getaAndSortPromos(): Promise<{
    activePromos: Array<Locator>;
    inactivePromos: Array<Locator>;
  }> {
  ...
  async checkPromoTourn({
    ...
    promoType: 'promo' | 'tournament';
    lang: string;
    expectedValue: string;
    section: 'promo' | 'tournament';
  }): Promise<boolean> {
    let receivedArray;
    let titleIsNotFound;
